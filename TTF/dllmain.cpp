...
typedef FLOAT(WINAPI* UpdateCustomFontMultiplier)(FLOAT mult);
HMODULE ddrawdll;
UpdateCustomFontMultiplier updateCustomFontMultiplierFontRendering;

float g_fontSizeMultiplier = 1.0f;

float updateDX11CustomMultiplier(float mult) {
    return ddrawdll != NULL && updateCustomFontMultiplierFontRendering ? updateCustomFontMultiplierFontRendering(mult) : 1.0;
}
...
void __fastcall G1_zCView_PrintChars(DWORD zCView, DWORD _EDX, int x, int y, zSTRING_G2& text)
{
    ...
    float oldMult = updateDX11CustomMultiplier(1);
    updateDX11CustomMultiplier(oldMult);

    fontHeight = fontHeight * oldMult;
    fontAscent = fontAscent * oldMult;
    ...
    for (int i = 0, len = text.Length(); i < len;)
    {
        ...
        else
        {
            ...
            unsigned int glyphWidth = std::get<0>(it->second) * oldMult;
            unsigned int glyphHeight = std::get<1>(it->second) * oldMult;
            int glyphLeft = std::get<2>(it->second) * oldMult;
            int glyphTop = std::get<3>(it->second) * oldMult;
            int glyphAdvance = std::get<9>(it->second) * oldMult;
...
BOOL WINAPI DllMain(HINSTANCE hInst, DWORD reason, LPVOID)
{
    if (reason == DLL_PROCESS_ATTACH)
    {
        ...
        ddrawdll = GetModuleHandleA("ddraw.dll");
        if (ddrawdll && GetProcAddress(ddrawdll, "GDX_AddPointLocator"))
        {
            g_GD3D11 = true;
            updateCustomFontMultiplierFontRendering = (UpdateCustomFontMultiplier)GetProcAddress(ddrawdll, "UpdateCustomFontMultiplier");
